...
# Model detail
parser.add_argument('--model', default='sglformer', type=str, metavar='MODEL',
...
parser.add_argument('--num-heads', type=int, default=None, metavar='N',
...
parser.add_argument('--acratio', type=float, default=0.5,)
...
def main():
    ...
    model = create_model(
        ...
        depths=args.layer, sr_ratios=1, T=args.time_step,
    )
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            # alpha = args.acratio - args.acratio*epoch/(num_epochs-100)
            # # alpha = args.acratio
            # # if epoch<=0.75*num_epochs:
            # # alpha = args.acratio - 0.05*(epoch//50)
            # if alpha>=0:
            #     for layer in model.modules():
            #         if isinstance(layer, AcFun):
            #             layer.alpha = alpha
            #             # print(layer.alpha)
            # else:
            #     for layer in model.modules():
